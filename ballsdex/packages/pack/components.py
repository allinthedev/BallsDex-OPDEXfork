...
from ballsdex.core.currency_models import CurrencySettings, Item
...
class ShopMenuSource(menus.ListPageSource):
    ...
    async def format_page(self, menu: Pages, page: Item) -> discord.Embed:
        ...
        emoji = (
            str(self.bot.get_emoji(page.emoji_id))
            if page.emoji_id
            else ""
        )
        currency_emoji = (
            self.bot.get_emoji(self.currency_settings.emoji_id) 
            if self.currency_settings.emoji_id 
            else ""
        )
        ...
        if countryballs:
            ...
            description += (
                f"Minimum Rarity: **{page.minimum_rarity}**\n"
                f"Maximum Rarity: **{page.maximum_rarity}**\n"
            )
